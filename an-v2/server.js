...
app.get('/api/collections', requireAuth, async(req,res)=>{
  try{
    const r=await pool.query(
      `SELECT c.*, COUNT(cp.photo_id)::int as photo_count
       FROM collections c
       LEFT JOIN collection_photos cp ON cp.collection_id=c.id
       WHERE c.owner=$1 GROUP BY c.id ORDER BY c.created_at DESC`,
      [req.username]);
    res.json(r.rows);
  }catch(err){res.status(500).json({error:err.message});}
});
...
app.post('/api/collections', requireAuth, async(req,res)=>{
  try{
    ...
    const now=Date.now();
    await pool.query('INSERT INTO collections(id,name,is_public,owner,created_at) VALUES($1,$2,$3,$4,$5)',
      [id,name.trim(),false,req.username,now]);
    res.status(201).json({id,name:name.trim(),photo_count:0,is_public:false,owner:req.username,created_at:now});
  }catch(err){res.status(500).json({error:err.message});}
});
...
app.put('/api/collections/:id', requireAuth, async(req,res)=>{
  try{
    ...
    const r=await pool.query(
      'UPDATE collections SET name=COALESCE($1,name),is_public=COALESCE($2,is_public) WHERE id=$3 AND owner=$4 RETURNING *',
      [name?.trim()||null,is_public,req.params.id,req.username]);
    if(!r.rows.length) return res.status(404).json({error:'Not found'});
    res.json(r.rows[0]);
  }catch(err){res.status(500).json({error:err.message});}
});
...
app.delete('/api/collections/:id', requireAuth, async(req,res)=>{
  try{
    const r=await pool.query('SELECT cover_cloudinary_id FROM collections WHERE id=$1 AND owner=$2',[req.params.id,req.username]);
    if(!r.rows.length) return res.status(404).json({error:'Not found'});
    if(r.rows[0].cover_cloudinary_id) await cloudinary.uploader.destroy(r.rows[0].cover_cloudinary_id).catch(()=>{});
    await pool.query('DELETE FROM collections WHERE id=$1',[req.params.id]);
    res.json({message:'ok'});
  }catch(err){res.status(500).json({error:err.message});}
});

app.get('/api/collections/:id/photos', requireAuth, async(req,res)=>{
  try{
    const col=await pool.query('SELECT * FROM collections WHERE id=$1 AND owner=$2',[req.params.id,req.username]);
    if(!col.rows.length) return res.status(404).json({error:'Not found'});
    const photos=await pool.query(
      ...
      [req.params.id]);
    res.json({...col.rows[0], photos:photos.rows});
  }catch(err){res.status(500).json({error:err.message});}
});
...
app.post('/api/collections/:id/photos', requireAuth, async(req,res)=>{
  try{
    ...
    if(!photo_ids?.length) return res.status(400).json({error:'No photos'});
    const col=await pool.query('SELECT * FROM collections WHERE id=$1 AND owner=$2',[req.params.id,req.username]);
    if(!col.rows.length) return res.status(404).json({error:'Not found'});
    ...
    // Auto cover from first photo if none set
    if(!col.rows[0].cover_url && photo_ids[0]){
      const ph=await pool.query('SELECT src FROM photos WHERE id=$1',[photo_ids[0]]);
      if(ph.rows.length) await pool.query('UPDATE collections SET cover_url=$1 WHERE id=$2',[ph.rows[0].src,req.params.id]);
    }
    res.json({added});
  }catch(err){res.status(500).json({error:err.message});}
});
...
app.post('/api/collections/:id/cover', requireAuth, upload.single('cover'), async(req,res)=>{
  try{
    if(!req.file?.mimetype.startsWith('image/')) return res.status(400).json({error:'Image required'});
    ...
    const result=await new Promise((resolve,reject)=>{
      const st=cloudinary.uploader.upload_stream({folder:'collection-covers',transformation:[{width:600,height:450,crop:'fill'}]},(e,r)=>e?reject(e):resolve(r));
      st.end(req.file.buffer);
    });
    await pool.query('UPDATE collections SET cover_url=$1,cover_cloudinary_id=$2 WHERE id=$3',[result.secure_url,result.public_id,req.params.id]);
